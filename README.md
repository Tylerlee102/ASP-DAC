...
- Verilator/Yosys/FPGA synthesis: pending local tool installation.
...
The local gate checks repository structure, event definitions, firmware benchmark pairs, static RTL contracts, replay parsing/comparison, six model-level bug capsules, baseline trace sizes, ablations, synthesis availability, and SVG figure generation. RTL compilation and synthesis are reported as unavailable when the required tools are not installed.
