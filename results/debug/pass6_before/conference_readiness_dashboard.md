...
Top blocker: Compiler-backed firmware, full-core mapped overhead, runtime-overhead baselines, and paper PDF build remain unavailable locally; full RTL record/replay is all-PASS for the evaluated rows and a scoped recorder-only iCE40 mapped row now passes.
...
| --- | --- | --- | --- | --- | --- | --- |
| G1. One-command reproduction | Yes | PARTIAL | `make reproduce`; `scripts/reproduce_all.ps1`; `scripts/run_all_tests.py`; `results/processed/summary.csv` | One command regenerates all claimed local evidence and records unavailable gates without silent skips. | Reproducibility | The Verilator replay simulator builds locally, full RTL replay is all-PASS, runtime current-harness rows are generated, and recorder-only iCE40 mapping has a PASS row; full submission reproduction still lacks compiler-backed firmware, full-core mapped overhead, and local paper PDF build. |
...
- G13 mapped synthesis/resource/timing has only a scoped recorder-only PASS row; full-core mapped overhead remains unavailable.
...
- A scoped recorder-only iCE40 mapped configuration, `replaycapsule_recorder_tiny`, placed and routed with 372 LUT, 205 FF, and 77.40 MHz Fmax.
