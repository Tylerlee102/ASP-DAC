...
The v2 replay-consume controller is tested RTL and is wired into the full-core Verilator wrapper as a host-streamed checker of observed boundary events against saved capsule words. It maps as a standalone controller, but it is not an autonomous replay datapath. The missing full-core pieces are capsule storage/streaming into the core wrapper, MMIO and IRQ replay muxing around the core, and an autonomous replay-mode controller. The artifact does not claim ASIC area or power, multi-hart behavior, DMA behavior, cache-coherence behavior, broad platform replay, or autonomous full-core hardware replay consume.
