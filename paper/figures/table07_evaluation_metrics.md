...
| firmware_running_rtl_replay_success_rate | MEASURED (rtl) | 45/45 (100.0%) | percent | rtl | Verilator record/replay rows across evaluated benchmark/variant/seed cases; not a mapped hardware timing or autonomous replay-engine claim. |
...
| hdl_frontend_pass_rate | MEASURED (rtl-smoke) | 30/30 (100.0%) | percent | rtl-smoke | Directed Icarus simulations, PicoRV32 wrapper smokes, and Verilator lint-only frontend checks. |
| directed_icarus_module_pass_rate | MEASURED (rtl-smoke) | 11/11 (100.0%) | percent | rtl-smoke | Standalone directed Icarus module simulations excluding PicoRV32 wrapper smoke rows. |
...
| generic_yosys_cell_overhead_percent | MEASURED (generic-yosys) | 295.95 | percent | generic-yosys | Generic unmapped cell-count overhead; mapped FPGA overhead is reported separately when same-target P&R rows pass. |
...
| mapped_tiny_baseline_fmax_mhz | MEASURED (mapped-fpga) | 157.48 | MHz | mapped-fpga | Scoped tiny iCE40 mapped result; not a full PicoRV32+ReplayCapsule mapped overhead claim. |
| mapped_recorder_tiny_lut | MEASURED (mapped-fpga) | 365 | lut | mapped-fpga | Scoped recorder-only iCE40 mapped result for replaycapsule_recorder_tiny; not a full PicoRV32+ReplayCapsule mapped overhead claim. |
...
| mapped_recorder_tiny_fmax_mhz | MEASURED (mapped-fpga) | 63.93 | MHz | mapped-fpga | Scoped recorder-only iCE40 mapped result for replaycapsule_recorder_tiny; not a full PicoRV32+ReplayCapsule mapped overhead claim. |
| mapped_tiny_lut_overhead_percent | MEASURED (mapped-fpga) | 693.48 | percent | mapped-fpga | scoped tiny-wrapper overhead; not a full-core overhead claim |
...
| mapped_tiny_bram_overhead | BLOCKED (mapped-fpga) | NA | brams | mapped-fpga | scoped tiny-wrapper overhead; not a full-core overhead claim; requires both same-target rows to PASS |
| mapped_tiny_fmax_delta_percent | MEASURED (mapped-fpga) | -59.40 | percent | mapped-fpga | scoped tiny-wrapper overhead; not a full-core overhead claim |
| full_core_mapped_lut_overhead_percent | MEASURED (mapped-fpga) | 143.75 | percent | mapped-fpga | full-core board-level mapped overhead from same-target place-and-route PASS rows |
| full_core_mapped_ff_overhead_percent | MEASURED (mapped-fpga) | 341.79 | percent | mapped-fpga | full-core board-level mapped overhead from same-target place-and-route PASS rows |
| full_core_bram_overhead | MEASURED (mapped-fpga) | 0.00 | brams | mapped-fpga | full-core board-level mapped overhead from same-target place-and-route PASS rows |
