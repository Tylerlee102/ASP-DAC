...
Current measured local results cover six model-level benchmark capsules, six RV32I firmware-sim benchmark capsules, the replay comparator, two directed Icarus SystemVerilog simulations, Verilator lint for `replay_capsule_top` and `picorv32_replaycapsule_wrapper`, and generic Yosys synthesis cell counts for both tops. The suites exercise sensor-threshold, interrupt-race, MMIO-ordering, stack-corruption, UART-command, and watchdog-timeout failures. Full firmware-running RTL simulation remains TODO because local `make`/C++ build support and a RISC-V compiler are still absent. FPGA-mapped LUT/FF/BRAM/Fmax numbers also remain TODO until an OpenROAD or vendor flow is run.
