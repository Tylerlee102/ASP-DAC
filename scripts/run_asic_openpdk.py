...
def _overhead_rows(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in rows:
        ...
        for metric in ("area_um2", "wns_ns", "power_mw"):
            ...
            if baseline.get("status") == "PASS" and row.get("status") == "PASS" and base is not None and replay is not None:
                delta = replay - base
                percent = (delta / base * 100.0) if base else None
                status = "PASS"
                notes = "OpenROAD global-routed Nangate45 physical-flow overhead from PASS rows"
            else:
                delta = None
                percent = None
                status = "BLOCKED"
            ...
            out.append(
                {
                    ...
                    "delta": f"{delta:.6f}" if delta is not None else "NA",
                    "percent_overhead": f"{percent:.2f}" if percent is not None else "NA",
                    "status": status,
                    "notes": notes,
                }
            )
