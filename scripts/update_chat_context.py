...
def section_evidence_status() -> list[str]:
    ...
    lines = [
        ...
        f"| ASIC/open-PDK | `{asic_summary_row.get('status', 'MISSING')}`; `{asic_area_pass}/{len(asic_area)}` Nangate45 synthesis-only area rows PASS; `{asic_summary_row.get('blocked_rows', 'NA')}` OpenROAD rows blocked; OpenROAD probe `{openroad_probe.get('status', 'MISSING')}`; local Docker `{docker_probe.get('status', 'MISSING')}`; OpenROAD Docker image `{docker_image_probe.get('status', 'MISSING')}`; WSL probe `{wsl_probe.get('status', 'MISSING')}`; area overheads: `{'; '.join(asic_area_claims) if asic_area_claims else 'NA'}` |",
        "",
    ]
    return lines
...
def section_claim_boundaries() -> list[str]:
    return [
        ...
        "- Nangate45 Yosys+ABC synthesis-only standard-cell area estimates, explicitly not placed/routed timing or power.",
        ...
        "- Placed/routed ASIC area, timing, power, or energy.",
...
def section_next_priorities() -> list[str]:
    return [
        ...
        "",
        "1. ASIC/open-PDK evidence: Nangate45 synthesis-only area rows, a local physical-tool probe, an OpenROAD Docker runner, and a GitHub Actions OpenROAD workflow now exist; remaining work is running that workflow or installing Docker/WSL so OpenROAD can produce placement/routing, timing, and power rows.",
        "2. Standalone replay flow polish: a reusable RTL self-replay SoC shell and expanded focused Icarus standalone matrix now exist, including explicit record/replay PicoRV32 interrupt-handler entry equality on IRQ rows; remaining work is broad board/peripheral integration beyond the focused memory/MMIO/IRQ/watchdog/profile2 shell.",
        "3. Second core breadth: FemtoRV32 Quark is now vendored with ReplayCapsule v1/v2 wrappers that pass the relevant focused checks, one compiler-backed capture smoke, a v1 RTL packet checker, a scoped v1 MMIO replay driver, a focused capture-profile replay matrix over base and expanded families, and scoped seeded v2 MMIO replay-consumer benchmark/config rows with replay-side host perturbations and property-signature equality; Hazard3 is now vendored as the pinned interrupt-capable candidate and has both a focused true-ISR firmware smoke and a 48-row seeded v2 MMIO+IRQ replay-consumer matrix across 8 workload families, 2 recorder configs, and 3 seeds. Remaining work is full-diagnostic all-commit Hazard3 replay only if the paper wants to claim that specific stronger config.",
        "4. Paper polish: preserve the RTL capture-store/self-replay and seeded Hazard3 matrix wording while keeping the Verilator orchestration, focused true-ISR smoke, and non-OS/application-suite limitations explicit.",
        "",
    ]
...
def section_file_map() -> list[str]:
    return [
        ...
        "- ASIC/open-PDK: `docs/asic_openpdk_evidence.md`, `docs/asic_physical_tool_probe.md`, `results/processed/asic_openpdk_yosys_area.csv`, `results/processed/asic_openpdk_yosys_area_overhead.csv`, `results/processed/asic_openpdk_summary.csv`, `results/processed/asic_physical_tool_probe.csv`, `scripts/run_asic_openroad_docker.ps1`, `scripts/materialize_nangate45_from_orfs.py`, and `.github/workflows/asic-openroad.yml`.",
