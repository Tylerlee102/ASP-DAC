#!/usr/bin/env python3
"""Run real-capsule v2 replay-consume prototype system tests."""
...
OUT_CSV = REPO_ROOT / "results/processed/replay_consumer_system_tests.csv"
RAW_DIR = REPO_ROOT / "results/raw/replay_consumer_system"
TESTBENCH = "tb/replay_consumer/tb_rcv2_system_replay_consumer.sv"
REAL_CAPSULE = "results/raw/rtl_capsules_v2/v2_core_interrupt_race_bug_failing_seed1.json"
...
def main() -> int:
    ...
    if not iverilog or not vvp:
        rows = [_blocked_row("replay_consumer_system_toolchain", "BLOCKED", "missing iverilog or vvp")]
        write_csv(OUT_CSV, FIELDS, rows)
        print(f"WROTE {rel(OUT_CSV)}")
        return 1

    simv = RAW_DIR / "tb_rcv2_system_replay_consumer.vvp"
    ...
    if c.returncode != 0:
        rows = [_blocked_row("replay_consumer_system_compile", "COMPILE_FAIL", f"see {rel(compile_log)}")]
        write_csv(OUT_CSV, FIELDS, rows)
        print(f"WROTE {rel(OUT_CSV)}")
        return c.returncode
    ...
    if not rows:
        rows = [_blocked_row("replay_consumer_system_run", "NO_RESULT_LINES", f"see {rel(run_log)}")]
    write_csv(OUT_CSV, FIELDS, rows)
    print(f"WROTE {rel(OUT_CSV)}")
    passed = sum(1 for row in rows if row["passed"] == "true")
    print(f"REPLAY_CONSUMER_SYSTEM_TESTS passed={passed} total={len(rows)}")
...
def _parse_results(text: str) -> list[dict[str, object]]:
    ...
    for line in text.splitlines():
        ...
        rows.append(
            {
                "test_name": test_name,
                "scope_label": "prototype_system_integration",
                "capsule_source": REAL_CAPSULE,
                "expected_result": expected,
                "actual_result": actual,
                "passed": "true" if passed == "1" else "false",
                "error_code": error_code,
                "notes": "real measured v2 core capsule words with targeted negative mutations",
            }
        )
    return rows


def _blocked_row(test_name: str, actual: str, notes: str) -> dict[str, object]:
    return {
        "test_name": test_name,
        "scope_label": "prototype_system_integration",
        "capsule_source": REAL_CAPSULE,
