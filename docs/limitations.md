...
- The local executable path includes a deterministic Python event model, an RV32I firmware interpreter, directed HDL checks, one PicoRV32 wrapper firmware smoke, and generic Yosys synthesis.
- The SoC RTL integration has one firmware smoke, not a completed benchmark-wide PicoRV32 firmware-running system.
...
- Model-level and firmware-sim replay are measured for six benchmarks; RTL firmware-running evidence is currently limited to one wrapper smoke.
