...
- The ECP5 mapped implementation is board-level recorder evidence, not an ASIC result. The selected v2 minimal recorder profile removes diagnostic logic at compile time and has a separate same-target overhead row; the core/hashed diagnostic rows still show high LUT/FF overhead.
