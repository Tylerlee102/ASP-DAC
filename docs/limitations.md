...
- The local executable path includes a deterministic Python event model, an RV32I firmware interpreter, directed HDL checks, twelve PicoRV32 wrapper firmware smokes, RTL-smoke capsule export self/negative/PC-context checks, and generic Yosys synthesis.
...
- Model-level and firmware-sim replay are measured for six benchmarks; RTL firmware-running evidence is currently limited to twelve wrapper smokes and twelve failing/fixed smoke-level capsule export self/negative/PC-context checks.
