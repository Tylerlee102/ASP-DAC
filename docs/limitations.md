...
- Model-level and firmware-sim replay are measured for six benchmarks; RTL firmware-running evidence is currently limited to twelve wrapper smokes, twelve failing/fixed smoke-level capsule export self-compare, missing-event, metadata-corruption, payload-corruption, and PC-context checks, and RTL/firmware-sim alignment rows.
