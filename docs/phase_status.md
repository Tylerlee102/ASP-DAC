...
| 2 | Record-side RTL modules | Complete | Event tap, classifier/slicer, property checker, hash/signature, MMIO/interrupt loggers, registers, replay-control, replay-mismatch, recorder, and capsule buffer RTL plus HDL/formal rows | No hardware replay-consume datapath is claimed |
...
| 4 | Host-driven replay and corruption rejection | Complete | `results/processed/full_rtl_replay.csv` reports 45/45 PASS; `results/processed/full_rtl_replay_negative.csv` reports 10 rejects, 0 unexpected accepts, 2 not-applicable rows | Replay consume is host-driven |
...
| 7 | Synthesis and mapped overhead | Complete | `results/processed/mapped_synthesis.csv`, `mapped_overhead.csv`, `mapped_recorder_presence.csv`, and `full_core_mapped_summary.csv` | ECP5 prototype is not area-optimized; no ASIC area/power claim |
...
limits explicit: single-hart RV32I interrupt/MMIO scope, host-driven replay
consume, no ASIC/power claim, and high prototype overhead.
