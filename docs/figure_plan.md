...
| `results/processed/formal_checks.csv` | Yes | Verification status | Depth-2 event-tap, depth-8 replay-control, depth-12 capsule-buffer, and depth-16 recorder SMTBMC BMC/cover targets pass. |
| `results/processed/rtl_capsule_exports.csv` | Yes | RTL capsule export status | Failing and fixed RTL smoke capsules decode to JSON, self-compare, fail missing-event negative checks through the replay comparator, and pass memory-event PC-context checks. |
