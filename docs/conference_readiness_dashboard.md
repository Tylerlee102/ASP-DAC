...
| Runtime overhead | PASS | baseline, disabled-recorder, and enabled-recorder rows in `results/processed/runtime_overhead_summary.csv`; v2 rows in `results/processed/runtime_scaling_v2_measured.csv` |
| Full-core mapped overhead | PASS | same-target ECP5 rows in `results/processed/mapped_synthesis.csv` and `results/processed/mapped_overhead.csv`; v2 representative full-core rows in `results/processed/mapped_scaling_v2_measured.csv` |
| Recorder presence | PASS | `results/processed/mapped_recorder_presence.csv` |
| v2 zero-fail gate | PASS | empty `results/processed/v2_zero_fail_bug_inventory.csv`; summary in `docs/v2_zero_fail_status.md` |
...
- Measured v2 zero-fail replay, workload scaling, runtime, representative mapped overhead, and new benchmark coverage.

## Forbidden Claims

- ASIC area or power.
- Autonomous full-core hardware replay-consume datapath.
